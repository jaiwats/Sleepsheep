...
Shader "Funly/Sky/StarrySky_Modified" { // Renamed shader slightly
    Properties{
        // Background Mode Control <<< NEW PROPERTY
        [Header(Background Options)]
        [Enum(Cubemap Only, 0, Gradient Only, 1, Cubemap Additive Gradient, 2)] _BackgroundMode("Background Mode", Int) = 0
            ...
            // Star Layer 1.
            [Header(Star Layer 1)]
            [Toggle(STAR_LAYER_1)] _EnableStarLayer1("Enable Layer 1", Float) = 1 // Added Toggle
            [NoScaleOffset]_StarLayer1Tex("Star 1 Texture", 2D) = "white" {}
            _StarLayer1Color("Star Layer 1 - Color", Color) = (1, 1, 1, 1)
            _StarLayer1Density("Star Layer 1 - Star Density", Range(0, .05)) = .01
            _StarLayer1MaxRadius("Star Layer 1 - Star Size", Range(0, .1)) = .007
            _StarLayer1TwinkleAmount("Star Layer 1 - Twinkle Amount", Range(0, 1)) = .775
            _StarLayer1TwinkleSpeed("Star Layer 1 - Twinkle Speed", float) = 2.0
            _StarLayer1RotationSpeed("Star Layer 1 - Rotation Speed", float) = 2
            _StarLayer1EdgeFade("Star Layer 1 - Edge Feathering", Range(0.0001, .9999)) = .2
            _StarLayer1HDRBoost("Star Layer 1 - HDR Bloom Boost", Range(1, 10)) = 1.0
            [HideInInspector]_StarLayer1DataTex("Star Layer 1 - Data Image", 2D) = "black" {}

            // Star Layer 2. - See property descriptions from star layer 1.
            [Header(Star Layer 2)]
            [Toggle(STAR_LAYER_2)] _EnableStarLayer2("Enable Layer 2", Float) = 1 // Added Toggle
            ...
            [Header(Star Layer 3)]
            [Toggle(STAR_LAYER_3)] _EnableStarLayer3("Enable Layer 3", Float) = 1 // Added Toggle
            ...
            _HorizonScaleFactor("Star Horizon Scale Factor", Range(0, 1)) = .7
                ...
                // Moon properties.
                [Header(Moon Options)]
                [Toggle(MOON)] _EnableMoon("Enable Moon", Float) = 1 // Added Toggle
        ...
        SubShader{
          ...
          Pass {
            CGPROGRAM
            // Removed GRADIENT_BACKGROUND feature, rely on _BackgroundMode uniform instead
            // Added toggles for layers/moon in properties, so keep these features
            ...
            // Background Mode Control <<< NEW UNIFORM
            int _BackgroundMode; // 0 = CubemapOnly, 1 = GradientOnly, 2 = CubemapPlusGradient
            ...
            // Gradient sky.
            // float _UseGradientSky; // This wasn't actually used, removed
            ...
            half3 CalculateHorizonGradient(float verticalPosition) {
              half fadePercent = smoothstep(_GradientFadeBegin, _GradientFadeEnd, verticalPosition);
              return lerp(_GradientHorizonColor.rgb, _GradientSkyColor.rgb, fadePercent); // Use .rgb
            }
      ...
      #ifdef MOON
            // Renamed function slightly for clarity
            half4 CalculateMoonColor(float3 pos, float3 moonPosition) {
              ...
              // Calculate alpha based on fade, then multiply final color
              half alpha = smoothstep(_MoonRadius, fadeEnd, fragDistance);
              return color * alpha; // Return color including alpha
            }
      #endif

            // --- MODIFIED FRAGMENT SHADER ---
            half4 frag(v2f i) : SV_Target {
                half3 finalBackground;

            // Calculate potential background components
            half3 cubemapColor = texCUBE(_MainTex, i.smoothVertex).rgb; // Use .rgb
            half3 gradientColor = CalculateHorizonGradient(i.verticalPosition);

            // Determine final background based on mode
            if (_BackgroundMode == 1) { // Gradient Only
                finalBackground = gradientColor;
            }
   else if (_BackgroundMode == 2) { // Cubemap Additive Gradient
    finalBackground = cubemapColor + gradientColor; // Additive blend
}
else { // Default: Cubemap Only (mode 0)
 finalBackground = cubemapColor;
}

            // --- Moon Calculation (Additive Overlay) ---
            // Calculate moon color separately if enabled
            ...
            if (distance(i.smoothVertex, _MoonComputedPositionData.xyz) <= _MoonRadius) {
                // Calculate the moon color (includes fade based on distance)
                moonColor = CalculateMoonColor(normalize(i.smoothVertex), _MoonComputedPositionData.xyz).rgb;
            }
        #endif

            // --- Star Calculation (Additive Overlay) ---
            ...
            // --- Final Composition ---
            // Add Moon and Stars to the final background
            half3 finalColor = finalBackground + moonColor + starColor;

            return half4(finalColor, 1.0f); // Ensure alpha is 1
        }
        ENDCG
      }
                }
                    // Keep the custom editor reference if you still want to use it,
                    // but be aware it might not show the new _BackgroundMode Enum correctly
                    // without being updated itself. If the Enum doesn't appear,
                    // you might need to temporarily remove this line to use the default inspector.
